...
export const newCalendarSlice = createSlice({
	...
	reducers: {
		...
		updateCalendar: (state, action) => {
			...
			if (calendarIndex !== -1) {
				state.inProgressCalendars[calendarIndex].calendarName =
					calendarName;
		...
		setActiveCalendar: (state, action) => {
			...
			if (calendarIndex !== -1) {
				const calendar = state.inProgressCalendars.splice(
					calendarIndex,
					1
				)[0];
		...
		setInactiveCalendar: (state, action) => {
			...
			const calendarIndex = state.activeCalendars.findIndex(
				(c) => c.calendarId === calendarId
			);

			if (calendarIndex !== -1) {
				const calendar = state.activeCalendars.splice(
					calendarIndex,
					1
				)[0];
		...
		deleteCalendar: (state, action) => {
			...
			state.inProgressCalendars = state.inProgressCalendars.filter(
				(c) => c.calendarId !== calendarId
			);
			state.activeCalendars = state.activeCalendars.filter(
				(c) => c.calendarId !== calendarId
			);
			state.inactiveCalendars = state.inactiveCalendars.filter(
				(c) => c.calendarId !== calendarId
			);
		},
	},
...
export const selectInProgressCalendars = (state) =>
	state.calendars.inProgressCalendars;
export const selectActiveCalendars = (state) => state.calendars.activeCalendars;
export const selectInactiveCalendars = (state) =>
	state.calendars.inactiveCalendars;
export const selectCalendarById = (calendarId) => (state) => {
	return (
		state.calendars.inProgressCalendars.find(
			(calendar) => calendar.calendarId === calendarId
		) ||
		state.calendars.activeCalendars.find(
			(calendar) => calendar.calendarId === calendarId
		) ||
		state.calendars.inactiveCalendars.find(
			(calendar) => calendar.calendarId === calendarId
		)
	);
};
