  ...
  return (
    <div className="toolbar-container">
      <div className={`nav-container ${navStatus ? "nav-show" : "nav-hide"}`}>
        ...
        <div
          className="nav-hideshow"
          onClick={() => setNavStatus((status) => !status)}
        >
          <div className="nav-hideshow-btn">
            <div
              className={`nav-arrow ${navStatus ? "nav-arrow-rotated" : ""}`}
            ></div>
          </div>
        </div>
        {newCalOpen && (
          <CalendarForm setEditMode={setEditMode} hideShow={hideShow} />
        )}
      </div>
    </div>
  );
